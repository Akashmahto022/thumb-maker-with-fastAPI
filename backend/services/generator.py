...
STYLES = {
    "bold_dramatic": (
        ...
        "The person's face should be prominent with dramatic expression."
        "The person's face should be prominent  with a dramatic expression."
    ),
    "clean_minimal": (
        "Create a clean, minimal Youtube thumnail with bright lighting,"
...
async def generate_single_thumbnail(thumbnail_id:str, prompt:str, headshot_url: str):
    ...
    try:
        ...
        with Session(engine) as session:
            ...
            # upload this image to image kit
        url = upload_file(
...
async def process_job(job_id: str):
    with Session(engine) as session:
        ...
        tasks = [
            generate_single_thumbnail(tid, job.prompt, headshot_url)
